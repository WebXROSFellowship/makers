...
const AFrame = props => {
  ...
  const updateInspectorData = async data => {
    const url = `/${ApiEndpoint_ApiEndpoint.UPDATE_INSPECTOR_DATA}`;
    var formdata = new FormData();
    formdata.append("file", new Blob([data]));
    const file_name = PAGE_SLUG + ".json";
    formdata.append("page", file_name);
    ApiConfig_HttpRequest.httpPost(url, formdata).then(result => {
